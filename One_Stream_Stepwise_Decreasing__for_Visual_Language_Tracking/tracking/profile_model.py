...
def evaluate_vit(model, text,template, search):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search,text),
...
def evaluate_vit_separate(model, text ,template, search):
    '''Speed Test'''
    T_w = 50
    T_t = 1000
    print("testing speed ...")
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":
        ...
        mask1 = torch.ones(bs,10)
        mask2 = torch.zeros(bs,30)
        mask = torch.cat((mask1,mask2),dim=1).to(device)
        # mask = torch.zeros(bs,40)
        tensor1 = torch.randint(1,1000,(bs,10)).int()
        tensor2 = torch.zeros(bs, 30).int()
        ...
        text = NestedTensor(tensor , mask).to(device)

        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_vit(model, text, template, search)
        else:
            evaluate_vit_separate(model, text, template, search)
